...
interface AuthContextValue {
  ...
  adminUsers: AppUser[];
  viewerUsers: AppUser[];
  login: (role: UserRole, userId: string, password: string) => { ok: boolean; message: string };
  logout: () => void;
...
function readPasswordStore(): Record<string, string> {
  ...
  const seeded = Object.fromEntries(appUsers.map((user) => [user.id, user.initialPassword]));
  ...
  const value = useMemo<AuthContextValue>(
    () => ({
      ...
      login: (role, userId, password) => {
        ...
        if (passwords[selected.id] !== password) {
